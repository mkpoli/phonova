...
**Standing constraints block** as in `phase-1.md` — copy the
block into every delegation. Voice-measure definitions:
`../../research/algorithms-and-validation.md` §5 (cite the exact subsection
in each delegation and paste its text in; references are not carried over).
...
### T4.3 · home screen / project manager
...
### T4.4 · spectrogram box selection + readout
...
### T4.5 · voice fixtures + oracle references
...
T4.1 and T4.2 are independent and start immediately (T4.2 needs
...
Gate T2.6 (outstanding) should close before T4.4 ships its
