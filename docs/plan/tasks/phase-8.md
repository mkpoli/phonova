...
Standing constraints and lane key as in `phase-1.md`, plus: `kimi` means a
Claude driver lane executing visual work through `opencode run -m
kimi-for-coding/k3`, verifying every diff against tests and `docs/DESIGN.md`.
...
### T8.1 · opus · container v2: groups, order, metadata
...
### T8.2 · opus · engine: rename, remove, band-filtered render, span export
...
### T8.3 · opus · streamed-source integration (task #15)
...
### T8.4 · kimi · library management (feedback A, B, E, G)
...
### T8.5 · kimi · switcher and recording flow (feedback C, D)
...
### T8.6 · kimi · navigation and view control (feedback H, I, J, K)
...
### T8.7 · opus · project and audio I/O (feedback F)
...
- Wave B runs strictly one task at a time in `apps/`, order T8.4 → T8.5 →
  T8.6, each behind the currently-running polish lane.
